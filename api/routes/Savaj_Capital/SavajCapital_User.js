...
router.post("/addsavajuser", async (req, res) => {
  try {
    const { userDetails } = req.body;

    const user = await SavajCapital_User.findOne({ email: userDetails.email });
    ...
    const newUser = new SavajCapital_User({
      ...userDetails,
      group_id: userId,
...
router.get("/savaj_user/:group_id", async (req, res) => {
  try {
    const group_id = req.params.group_id;
    const user = await SavajCapital_User.findOne({ group_id });
...
router.get("/SavajCapital_User", async (req, res) => {
  try {
